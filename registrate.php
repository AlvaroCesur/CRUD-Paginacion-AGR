...
// 	$usuario = stripslashes($usuario);
	...
	if (empty($usuario) or empty($password) or empty($password2)) {
		...
		$statement = $conexion->prepare('SELECT * FROM login_crud WHERE usuario = :usuario LIMIT 1');
	...
	// Comprobamos si hay errores, si no entonces agregamos el usuario y redirigimos.
	if ($errores == '') {
		$statement = $conexion->prepare('INSERT INTO login_crud (id, usuario, pass) VALUES (null, :usuario, :pass)');
